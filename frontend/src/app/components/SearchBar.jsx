...
const SearchBar = ({ mobile = false }) => {
  ...
  // When a category filter is active, show packages of that category
  // Otherwise show all category groups
  const hotDestinations = useMemo(() => {
    ...
    packages.forEach((pkg) => {
      ...
      if (!catName) return;
      if (!map[catName]) {
        // Find the category image from categories API for better image quality
  ...
  // If a category is selected, show individual package cards for that category
  // Otherwise, show the grouped category cards
  const hotDestinationsView = useMemo(() => {
    if (activeCategories.length > 0) {
      // Show individual packages for the selected categories
      return packages.filter((pkg) =>
        activeCategories.includes(pkg.category?.name),
      );
    }
    return null; // null means show grouped category view
  ...
  const filteredPackages = useMemo(() => {
    return packages.filter((pkg) => {
      ...
      if (activePrices.length > 0) {
        const range = priceRanges.find((r) => activePrices.includes(r.label));
        if (
          range &&
  ...
  }, [packages, searchText, filters, selectedYear, activeCategories]);
  ...
  // Navigate to package detail page and close modal
          ...
          <div className="w-full max-w-6xl bg-white text-black rounded-xl shadow-lg flex flex-col max-h-[95vh]">
            ...
            <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="space-y-6 bg-yellow-50 p-4 rounded-lg">
                {/* Best Seasons — dynamic from unique category names in packages */}
                <div>
                ...
                <div>
                  ...
                  {isLoading ? (
                    ...
                    // Show individual packages for selected category
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  {activeTab === "month" ? (
                    ...
                    // Occasions — dynamic from tourType
                    <div className="flex flex-wrap gap-2">
                      {occasions.length === 0 ? (
                        <p className="text-xs text-gray-400">
