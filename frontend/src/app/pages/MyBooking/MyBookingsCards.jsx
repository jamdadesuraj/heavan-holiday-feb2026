...
const EditBookingModal = ({ isOpen, onClose, booking }) => {
  const [updateBookingTravelers, { isLoading, isSuccess, reset }] =
    useUpdateBookingTravelersMutation();
  ...
  const handleAddTraveler = (type) => {
    ...
    // Auto-open the newly added traveler form
  ...
  const handleRemoveTraveler = (index) => {
    ...
    // Prevent removing lead traveler directly
    if (traveler.isLeadTraveler) {
      toast.error(
  ...
  // ========== CHANGE LEAD TRAVELER FUNCTION ==========
  const handleChangeLeadTraveler = (index) => {
    const traveler = formData.travelers[index];

    // Only adults can be lead travelers
    ...
    setFormData((prev) => ({
      ...
      travelers: prev.travelers.map((t, i) => ({
        ...
        // Add email/phone fields if becoming lead traveler
  ...
  const validateForm = () => {
    ...
    // Check if there's at least one adult
    const adultCount = formData.travelers.filter(
      (t) => t.type === "Adult",
    ).length;
    if (adultCount === 0) {
    ...
    // Check if there's exactly one lead traveler
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 p-6">
            ...
            <div className="lg:col-span-1">
              <div className="bg-white p-5 rounded-xl shadow border border-gray-200 sticky top-4">
                ...
                {/* Show pricing change warning */}
                {travelerCounts.total !== booking.travelerCount?.total && (
                  <div className="bg-yellow-50 border border-yellow-200 rounded p-3 mb-3">
                    <p className="text-xs text-yellow-800">
                      <strong>Note:</strong> Traveler count changed from{" "}
                      {booking.travelerCount?.total} to {travelerCounts.total}.
...
const CancelBookingModal = ({ isOpen, onClose, booking }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      onClose(); // Close first
      toast.success("Booking cancelled successfully!"); // Then alert
    } catch (error) {
      toast.error(
...
const MyBookingsCards = () => {
  ...
  const Bookings = useMemo(() => {
    if (!myBookings?.data?.bookings) return [];
    return myBookings.data.bookings.filter((item) => item.user === userId);
  }, [myBookings, userId]);
